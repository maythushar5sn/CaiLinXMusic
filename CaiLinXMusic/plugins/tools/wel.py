...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("CaiLinXMusic/assets/CaiLinXMusic12.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((1157, 1158))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('CaiLinXMusic/assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('CaiLinXMusic/assets/font.ttf', size=60)
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        pic = "CaiLinXMusic/assets/upic.png"
